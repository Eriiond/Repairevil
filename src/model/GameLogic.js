...
export class GameLogic {
    ...
    static update(gameState) {
        this.updateMoney(gameState);
        this.updateShips(gameState);
        this.updateFights(gameState);
    }
    ...
    static updateShips(gameState) {
        for (let i = 0; i < gameState.universe.planets.length; i++) {
            ...
            if (element.population.virus > 0) {
                element.population.virus += Math.floor(element.growthRate * virusFactor * (gameState.level % 5 + 1));
            }
            if (element.population.player > 0) {
    ...
    static updateFights(gameState) {
        for (let i = 0; i < gameState.universe.spaceConnections.length; i++) {
            ...
            if (startPlanet.population.default == 0 && this.isSpreading(element.sendPorbability)) {
                this.spread(startPlanet, endPlanet);
            }
            if (endPlanet.population.default == 0 && this.isSpreading(element.sendPorbability)) {
    ...
    static spreadVirus(fromPlanet, toPlanet) {
        var shipFleet = Math.floor(startPlanet.population.virus * element.startPlanet.spreadRate / 100);
        fromPlanet.population.virus -= shipFleet;
        fightPlanetWithVirus(toPlanet, shipFleet);
    }

    static spreadPlayer(fromPlanet, toPlanet) {
        var shipFleet = Math.floor(startPlanet.population.player * element.startPlanet.spreadRate / 100);
        fromPlanet.population.virus -= shipFleet;
    ...
    static fightPlanetWithVirus(attackedPlanet, shipFleet) {
        // planet is unowned
        if (attackedPlanet.population.virus == 0 && attackedPlanet.population.player == 0 && attackedPlanet.population.default == 0) {
        ...
        if (attackedPlanet.population.player > 0) {
            ...
            if (attackedPlanet.population.player < 0) {
                attackedPlanet.population.virus += attackedPlanet.population.player * -1;
        ...
        if (attackedPlanet.population.default > 0) {
            ...
            if (attackedPlanet.population.default < 0) {
                attackedPlanet.population.virus += attackedPlanet.population.default * -1;
    ...
    static fightPlanetWithPlayer(attackedPlanet, shipFleet) {
        ...
        if (attackedPlanet.population.virus > 0) {
            attackedPlanet.population.virus -= shipFleet;
            if (attackedPlanet.population.virus < 0) {
                attackedPlanet.population.player += attackedPlanet.population.virus * -1;
        ...
        if (attackedPlanet.population.default > 0) {
            attackedPlanet.population.default -= shipFleet;
            if (attackedPlanet.population.default < 0) {
                attackedPlanet.population.player += attackedPlanet.population.default * -1;
                attackedPlanet.population.default = 0;
            }
        }
    }
